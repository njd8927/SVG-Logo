...
// render method to generate the circle 
    ...
    render() {
        ...
        <circle cx="75" cy="75" r="30" fill="${this.color}"/>
        <text x="75" y="80" font-size="27" text-anchor="middle" fill="${this.color}">${this.text}</text>
        </svg>`
    }
};
...
// render method to generate the triangle
    ...
    render() {
        return `<svg version="1.1" width="300" height="200" xmlns="https://www.w3.org/TR/SVG/">
        <polygon points="150,25 250,150 50,150" fill="${this.color}"/>
        <text x="145" y="100" font-size="27" text-anchor="middle" fill="${this.color}">${this.text}</text>
        </svg>`
    }
};
...
// render method to generate the square
    ...
    render() {
        return `<svg version="1.1" width="300" height="200" xmlns="https://www.w3.org/TR/SVG/">
        <rect x="90" y="90" width="100" height="100" fill="${this.color}"/>
        <text x="140" y="150" font-size="27" text-anchor="middle">${this.text}</text>
        </svg>`
    }
};
